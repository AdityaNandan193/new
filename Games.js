    ...
    const fetchEventData = async () => {
      ...
      setSelectedDay(day);
        ...
        const eventsSnapshot = await get(
          ref(db, `events/Technical Events/${day}/${selectedEventName}`)
        );
    ...
    const fetchEventDataDay3 = async () => {
      try {
        ...
        setSelectedDay(day);
          ...
          const eventsSnapshot = await get(
            ref(db, `events/Technical Events/${day}/${eventName}`)
          );
      ...
      if (user) {
        ...
        const userName = userSnapshot.val().Name;
          ...
          const userTechnicalRegistrationSnapshot = await get(
            ref(FIREBASE_DB, `users/${user.email.replace(/[^a-zA-Z0-9]/g, '_')}/isRegisteredTechnical`)
          );
  
          if (userTechnicalRegistrationSnapshot.exists()) {
            Alert.alert(
              'Already Registered',
              'You have already registered for a technical event. You cannot register for another event.'
            );
            return;
          }
   ...
   return; // Do not proceed with registration
    ...
    if (userRegistrationsSnapshot.exists()) {
      ...
      if (hasRegisteredOnThisDay) {
        ...
        await set(
          ref(db, `events/Technical Events/${selectedDay}/${selectedEvent}/slotCounter`),
          slotsLeft - 1
        );
        ...
        await set(ref(db, `users/${user.email.replace(/[^a-zA-Z0-9]/g, '_')}/isRegisteredTechnical`), true);
  ...
  return (
    <SafeAreaView style={styles.conatainer}>
      ...
      />
        ...
        <Text
          style={{ fontWeight: 'bold', fontSize: 20, marginVertical: '2%',marginLeft: "1%" }}
        >
          Description:{' '}
        </Text>
      ...
      <View style={styles.registrationButtonContainer}>
        {user && slotsLeft > 0 ? (
          <TouchableOpacity
            ...
            onPress={() => handleRegistration(selectedDay, selectedEvent ,'Technical')}

          >
            <Text style={styles.registrationButtonText}>Register</Text>
          </TouchableOpacity>
        ) : (
          <View style={styles.eventFullText}>
